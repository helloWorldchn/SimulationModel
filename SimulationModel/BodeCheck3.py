...
# 选择超前校正器的零点和极点
# 找到需要增加相角的频率范围
phase_deg = np.unwrap(np.degrees(phase))
target_freq = omega[np.argmin(np.abs(phase_deg + 180 - desired_pm))]

# 计算超前校正器的参数
alpha = (1 + np.sin(np.radians(phase_margin_needed))) / (1 - np.sin(np.radians(phase_margin_needed)))
tau = 1 / (target_freq * np.sqrt(alpha))

# 创建超前校正器
C = ctrl.TransferFunction([1, tau], [1, tau / alpha])
